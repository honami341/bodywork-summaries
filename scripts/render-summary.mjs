...
function validate(data, source) {
  ...
  return {
    ...
    cards: cards.map((card, index) => {
      ...
      return {
        id: index + 1,
        section: typeof card.section === "string" && card.section.trim()
          ? requireText(card.section, `cards[${index}].section`, 40)
          : "",
...
data.cards.forEach((card,index)=>{
  if(card.section&&card.section!==currentSection){
    cards.append(el("div","section-title",card.section));
